...
def name_project(message):
    ...
    data = [user_id, name]
    bot.send_message(message.chat.id, "⛓️‍💥Введите ссылку на проект✏️")
    bot.register_next_step_handler(message, link_project, data=data)

def link_project(message, data):
    data.append(message.text)
    statuses = [x[0] for x in manager.get_statuses()] 
    bot.send_message(message.chat.id, "✏️Введите текущий статус проекта✏️", reply_markup=gen_markup(statuses))
...
def delete_project(message, projects):
    ...
    if project not in projects:
        bot.send_message(message.chat.id, '☹️У тебя нет такого проекта, попробуй выбрать еще раз!☹️', reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, delete_project, projects=projects)
        return
...
def update_project(message):
    ...
    if projects:
        projects = [x[2] for x in projects]
        bot.send_message(message.chat.id, "📋Выбери проект, который хочешь изменить✏️", reply_markup=gen_markup(projects))
...
def update_project_step_2(message, projects):
    ...
    if message.text == cancel_button:
        ...
        return
    if project_name not in projects:
        bot.send_message(message.chat.id, "⚠️Что-то пошло не так!⚠️ Выбери проект, который хочешь изменить еще раз:", reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, update_project_step_2, projects=projects )
        return
